...
export async function createEphemeralUser(name: string): Promise<TestUser & { password: string }> {
  ...
  const { data, error } = await serviceClient.auth.admin.createUser({
    ...
    email_confirm: true,
    user_metadata: {
      username: `${name}_${suffix}`.replace(/[^a-z0-9_]/gi, "_"),
    },
  });
...
/**
 * Fully delete an ephemeral user and all their data.
 * Deletes data in FK-safe order, then removes auth user.
 */
export async function deleteEphemeralUser(userId: string): Promise<void> {
  ...
  await serviceClient
    ...
    .or(`requested_by.eq.${userId},requested_to.eq.${userId}`);
  await serviceClient.from("challenges").delete().eq("creator_id", userId);
  await serviceClient.from("profiles_public").delete().eq("id", userId);
