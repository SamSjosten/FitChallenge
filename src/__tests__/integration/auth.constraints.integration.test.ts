...
(shouldSkip ? describe.skip : describe)("Auth Database Constraints (Integration)", () => {
  ...
  describe("profiles.username UNIQUE constraint", () => {
    ...
    it("should reject duplicate usernames with error code 23505", async () => {
      ...
      const errorMessage = (error2 as Error | null)?.message?.toLowerCase() ?? "";
      const hasError =
        error2 !== null ||
        user2.user === null ||
    ...
    it("should reject duplicate username on direct insert", async () => {
      ...
      const { error: insert1Error } = await serviceClient.from("profiles").insert({
        id: testId1,
      ...
      if (insert1Error) {
        console.log("Skipping direct insert test - FK constraint prevents test");
        return;
      }

      try {
        // Try duplicate insert - should fail with unique violation
        const { error: insert2Error } = await serviceClient.from("profiles").insert({
          id: testId2,
          username: duplicateUsername,
        });
        ...
        const errorMsg = (insert2Error as Error | null)?.message?.toLowerCase() ?? "";

        expect(
          errorCode === "23505" || errorMsg.includes("duplicate") || errorMsg.includes("unique"),
        ).toBe(true);
      } finally {
        // Cleanup
        await serviceClient.from("profiles").delete().eq("username", duplicateUsername);
      }
    });
  });
